...
export async function load({ params, fetch }) {
  ...
  const blockLabel = blockConfig?.label ?? null;
  const catLabel = block && cat && blockConfig?.categories?.[cat] ? blockConfig.categories[cat] : null;

  const imageUrl = raw.image
    ? `${PUBLIC_CMSURL}/assets/${raw.image}?width=1200&quality=80&format=webp`
    : null;
  ...
  try {
    ...
    console.warn('Could not load quiz for article:', err);
  }
