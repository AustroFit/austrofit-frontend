...
export async function syncSteps(options: {
  ...
  // Default: always 90 days. Server dedup (skipped:true) handles already-synced days efficiently.
  // A smaller explicit value (e.g. days:1 from the SW trigger) is still respected.
  const { days = 90, mode = 'automatic', onProgress } = options;
